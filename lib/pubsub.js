...
function pushed(model, opt) {
  if (!model || !opt.channel || !channels[opt.channel]) return
  channels[opt.channel] && channels[opt.channel].forEach(function(x) {
...
module.exports = function(options) {
  ...
  return function(client, con) {
    ...
    this.publish = function(model, opt, fn) {
      if (!model || !opt.channel) {
        return (opt.error && opt.error('missing parameters'))
      }
      if (pub) {
        pub.publish(opt.channel, JSON.stringify({
          model: model
        , options: opt
        }))
      } else {
